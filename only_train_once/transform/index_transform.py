...
def index_transformation_param_group(
    indexes_by_groups, transformation_type, param_group
):
    if transformation_type == TensorTransform.MULTIHEAD_HEADDIM:
        return index_transformation(
            indexes_by_groups,
            transformation_type,
            num_heads=param_group["num_heads"],
            head_dim=param_group["head_dim"],
        )
    elif (
        ...
        return index_transformation(
            indexes_by_groups, transformation_type, head_dim=param_group["head_dim"]
        )
    else:
...
def index_transformation(
    indexes_by_groups, transformation_type, num_heads=1, head_dim=1
):
    if (
        transformation_type == TensorTransform.NO_UPDATE
        or transformation_type == TensorTransform.NO_PRUNE
        or transformation_type == TensorTransform.BASIC
        or transformation_type == TensorTransform.ACCESSORY
        or transformation_type == TensorTransform.TRANSPOSE
    ):
